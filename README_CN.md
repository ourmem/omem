...
| | 对象存储 | 阿里云 OSS 或 S3 兼容存储，支持 ECS RAM 角色 |
...
默认情况下 ourmem 将数据存储在本地磁盘。如需持久化和可扩展性，可配置阿里云 OSS 或任何 S3 兼容的对象存储：

```bash
# 添加到你的环境变量
OMEM_OSS_BUCKET=your-bucket           # 启用 oss:// 方案（推荐阿里云用户使用）
...
> OSS 优先于 S3。如果同时设置了 `OMEM_OSS_BUCKET` 和 `OMEM_S3_BUCKET`，将使用 OSS。
