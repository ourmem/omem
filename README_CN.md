...
**OpenCode:** `"plugin": ["@ourmem/opencode"]` 加到 opencode.json
**Claude Code:** `/plugin marketplace add ourmem/omem`
**OpenClaw:** `openclaw plugins install @ourmem/openclaw`
**MCP:** `npx @ourmem/mcp`
...
curl -sX POST "$OMEM_API_URL/v1/memories" \
  ...
  -d '{"content": "我偏好深色模式", "tags": ["preference"]}'
```
