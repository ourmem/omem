...
pub async fn get_memory(
    ...
    // Lazy propagation: if this is a shared copy whose source has moved on,
    // pull the current source content+vector before returning so readers never
    // see a stale snapshot. Opt-in via OMEM_AUTO_REFRESH_SHARES; source-deleted
    // copies are left as-is (handled by the explicit unshare/stale path).
